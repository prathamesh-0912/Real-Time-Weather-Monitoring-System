...
def prepare_temperature_data(interval):
    if interval == "Daily":
        xds = xr.open_mfdataset('C:\\Users\\lawan\\OneDrive\\Documents\\Pranjal\\New folder\\data\\vComponentOfWind\\*.nc', parallel=False)
        xds_daily = xds.resample(time='1D').mean()
        return xds_daily['V_GRD_L100'] 
    elif interval == "6-Hourly":
        xds = xr.open_mfdataset('C:\\Users\\lawan\\OneDrive\\Documents\\Pranjal\\New folder\\data\\vComponentOfWind\\*.nc', parallel=False)
        return xds['V_GRD_L100']
    elif interval == "Yearly":
        xds = xr.open_mfdataset('C:\\Users\\lawan\\OneDrive\\Documents\\Pranjal\\New folder\\data\\vComponentOfWind\\*.nc', parallel=False)
        data  = xds.resample(time='YS').mean()
        return data['V_GRD_L100'] 
...
def save_temperature_data(interval, folder_path):
    ...
    file_path = os.path.join(folder_path, 'seasonal_VCOW_data.nc')
    temperature_data.to_netcdf(file_path)

if __name__ == '__main__':
    folder_path = 'C:\\Users\\lawan\\OneDrive\\Documents\\Pranjal\\New folder\\data\\VCOW'
